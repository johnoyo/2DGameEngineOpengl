...
void Game::load_level(Vertex_Array* vertex, std::string & level_path, float width, float height, float character_scale)
{
	...
	for (int k = 0; k < p.size(); k++) {
		...
		/* NOTE: here the color vertex gets ignored, it doesnt get used in the shader,
				 maybe in the future i may implement quads with either texture or color or both(tinted texture)*/
		...
		world[p.at(k).j * w + p.at(k).i].exist = true;
